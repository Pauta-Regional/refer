// ----------------------------------------------------------
// ZeeNamesAlternative.cs from "Programming in the Key of C#"
// ----------------------------------------------------------
using System;

class ZeeNamesAlternative
{
    static void Main()
    {
        ...
        char[] achSearch = {'z', 'Z'};
        string[] astrTemp = new string[astrMyFriends.Length];

        // Copy and count up the names with 'z' or 'Z' in them.

        for (int i = 0; i < astrMyFriends.Length; i++)
            if (astrMyFriends[i].IndexOfAny(achSearch) != -1)
                astrTemp[iNamesWithZees++] = astrMyFriends[i];

        // Next, declare an array of that size

        string[] astrMyFriendsWithZees = new string[iNamesWithZees];

        // Transfer the names from the temp array

        for (int i = 0; i < iNamesWithZees; i++)
            astrMyFriendsWithZees[i] = astrTemp[i];
